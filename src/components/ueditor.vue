...
<template>
  <script :id="randomId" name="content" style="height:400px" type="text/plain"></script>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      //randomId: 'editor', // 随机id会导致草稿箱保存内容不可用 由于业务需求且不存在一个页面多个编辑器，使用固定id
      instance: null,
  ...
  computed:{
    uin(){
      return this.$store.getters.userInfo.uin
    },
    randomId(){
      return 'editor_' + this.uin
    }
  },
  created () {
